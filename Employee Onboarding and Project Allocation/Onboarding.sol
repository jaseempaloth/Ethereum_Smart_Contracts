...
abstract contract Onboarding {
    ...
    function join(uint256 _id, string memory _name, string memory _email, uint256 _salary) public virtual {
        employee = Employee(_id,_name, _email, _salary);
    }
...
contract Allocation is Onboarding {
    ...
    function sendWelcomeEmail() internal view returns (string memory)  {
        return string.concat("Send welcome email to employee with email:", employee.email);
    }
}